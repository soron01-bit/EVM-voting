...
function Admin({ user, onLogout }) {
  ...
  const generateVoterId = () => {
    // Generate a unique voter ID with format: V + 4-digit number + random letter
    const randomNum = Math.floor(Math.random() * 9000) + 1000; // 1000-9999
    const randomLetter = String.fromCharCode(65 + Math.floor(Math.random() * 26)); // A-Z
    return `V${randomNum}${randomLetter}`;
  };

  const handleAutoGenerateVoter = async () => {
    setRegistrationMessage('');
    setRegistrationLoading(true);
    ...
    try {
      ...
      do {
        newId = generateVoterId();
        attempts++;
        if (attempts >= maxAttempts) {
          throw new Error('Unable to generate unique voter ID after multiple attempts');
        }
      } while (registeredVoters.some(v => v.voterId === newId));

      // Register the auto-generated voter
      const registeredVotersRef = collection(db, 'registered_voters');
      await addDoc(registeredVotersRef, {
        voterId: newId,
        registeredAt: new Date(),
      ...
      setRegistrationMessage({
        type: 'success',
        text: `Auto-generated Voter ID "${newId}" registered successfully!`
      });
      setNewVoterId(''); // Clear manual input
      setTimeout(() => setRegistrationMessage(''), 4000);
    } catch (error) {
      console.error('Error auto-generating voter:', error);
      setRegistrationMessage({
        type: 'error',
        text: error.message || 'Error auto-generating voter ID'
      });
    } finally {
      setRegistrationLoading(false);
    }
          ...
          <div className="register-section">
            <div className="register-card">
              ...
              <p className="register-description">
                Add voter IDs that will be allowed to participate in the election. You can either manually enter IDs or auto-generate unique ones.
              </p>

              <div className="registration-options">
                <div className="option-card">
                  <h3>🔢 Manual Registration</h3>
                  <p>Enter a custom voter ID</p>
                  <form onSubmit={handleRegisterVoter} className="register-form">
                    ...
                    <button
                      ...
                      className="register-button manual"
                    >
                ...
                <div className="option-card">
                  <h3>🎲 Auto-Generate ID</h3>
                  <p>Generate a unique voter ID automatically</p>
                  <button
                    onClick={handleAutoGenerateVoter}
                    disabled={registrationLoading}
                    className="register-button auto"
                  >
                    {registrationLoading ? 'Generating...' : '⚡ Auto-Generate Voter ID'}
                  </button>
                </div>
              </div>
